...
def max_degree_cent(G): 
    Graph = nx.Graph(G)

    dc = nx.algorithms.centrality.degree_centrality(Graph)
    max_arg = sorted(dc.items(), key=lambda x: x[1], reverse = True)
    data = []
    for key in max_arg:
        data.append(key[0])
    return data[:100]
...
# reshape_mx(mx, shape)功能：将稀疏矩阵转换为指定形状的稀疏矩阵
...
# sparse_mx_to_khopsgraph(sp_mx)功能：将稀疏矩阵转换为k-hop图
def sparse_mx_to_khopsgraph(sp_mx):
    ...
    for i in range(len(graph)):
        onehops_dict[i] = [k for k in graph.neighbors(i)]
    return onehops_dict
